...
Function Set-LocalPrinters {
    ...
    if (($netconn.PingSucceeded) -and ($netConn.NameResolutionSucceeded)) {
        ...
        if (Get-Printer -Name "\\srvprint\009 imprimante chariot a" -ErrorAction SilentlyContinue) {
...
$printers = @(
    [PSCustomObject]@{
        Printer = "PR009mobilea01"
        Server = "srvprint"
    }
)
#endregion
#region Install printers
Start-Transcript -Path "C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\$printerName-install.log" -Force
